    ...
    let solve (g:Graph.t) : Graph.t =
      ...
      *)
      ...
      while not @@ Graph.NodeS.is_empty !w do
        ...
        let old_out = Graph.out !gref n in
        let ins = Fact.combine (List.map (Graph.out !gref) (Graph.NodeS.elements (Graph.preds !gref n))) in
        let new_out = Graph.flow !gref n ins in
        gref := Graph.add_fact n new_out !gref;
        if not (0 = Fact.compare old_out new_out) then
          Graph.NodeS.fold (fun x _ -> w := Graph.NodeS.add x !w) (Graph.succs !gref n) ()
      done;
      !gref
