...
let better_layout (f:Ll.fdecl) (live:liveness) : layout =
  ...
  *)
  ...
  let max_deg_node (degs:int UidM.t) : (uid*int) option = UidM.fold (fun uid deg x ->
            match x with
            | None -> Some (uid, deg)
            | Some (uid2, deg2) -> if deg > deg2 then Some (uid, deg) else Some (uid2, deg2)
          ) degs None in

  let rec kempe (graph : (UidS.t ref) UidM.t) (k:int) (pre_coloring:(Ll.uid * Alloc.loc) list) (preferred_colors:(Ll.uid * LocSet.t) list) : (Ll.uid * Alloc.loc) list =
    if UidM.cardinal graph = 0 then [] else
    let not_precolored = UidM.filter (fun u _ -> not (List.mem_assoc u pre_coloring)) graph in
    if UidM.cardinal not_precolored = 0 then
      (* All nodes pre-colored *)
      pre_coloring
    else
    let not_prec_degs = UidM.map (fun sref -> UidS.cardinal !sref) not_precolored in
    let victim = match max_deg_node @@ UidM.filter (
      fun u d -> d < k
    ) not_prec_degs with
      | None -> begin match max_deg_node not_prec_degs with
        | None -> failwith "wtf: cannot find node to spill"
        | Some (victim,_) -> 
          victim
        end
      | Some (victim, _) -> 
        victim
    in
    let g2 = UidM.remove victim graph in
    UidM.iter (fun _ sref -> sref := UidS.remove victim !sref) g2;
    let coloring = kempe g2 k pre_coloring preferred_colors in
    let neighbor_colors = List.map (fun u -> List.assoc u coloring) (UidS.elements !(UidM.find victim graph)) in
    let avail_colors = LocSet.filter (fun i -> not (List.mem i neighbor_colors)) pal in
    let pref = List.assoc_opt victim preferred_colors in
    let color_opt = match pref with
      | Some pref_colors -> 
        LocSet.choose_opt @@ LocSet.inter avail_colors pref_colors
      | None -> None
    in
    match color_opt with
      | Some x -> (victim, x) :: coloring
      | None -> begin match LocSet.choose_opt avail_colors with
        | None -> (victim, spill()) :: coloring
        | Some x -> (victim,x) :: coloring
      end
  in

  let interference_graph : (UidS.t ref) UidM.t = fold_fdecl
    (fun u (x,_) -> UidM.add x (ref UidS.empty) u)
    (fun u _ -> u)
    (fun u (x,i) ->
      if insn_assigns i then UidM.add x (ref UidS.empty) u else u
    )
    (fun u _ -> u)
    UidM.empty f in
  
  (* Add edges: iff uid1 and uid2 are live at the same time (e.g. both in some out-set) *)
  ignore @@ fold_fdecl
    (fun _ _ -> ())
    (fun _ _ -> ())
    (fun _ (x,i) ->
      try
      let out_set = live.live_out x in
      UidS.iter (fun uid1 -> UidS.iter (fun uid2 ->
        if not (0 = String.compare uid1 uid2) then
          let s1ref = UidM.find uid1 interference_graph in
          let s2ref = UidM.find uid2 interference_graph in
          s1ref := UidS.add uid2 !s1ref;
          s2ref := UidS.add uid1 !s2ref
      ) out_set) out_set
      with Not_found -> failwith "wtf: uid's in out-list not found in graph"
    )
    (fun _ _ -> ())
    () f;
  
  let k = Alloc.LocSet.cardinal pal in
  (* Pre-color function parameters and never remove / simplify them *)
  let pre_coloring : (Ll.uid * Alloc.loc) list = fold_fdecl
  ...
  let preferred_colors : (LocSet.t UidM.t) = fold_fdecl
    (fun c _ -> c)
    (fun c _ -> c)
      ...
      | Call (_, _, args) ->
        ...
        List.fold_right (fun (op, i) c' -> match op with
          | Id uid -> if i >= 6 then c' else
            let prev = match UidM.find_opt uid c' with
              | None -> LocSet.empty
              | Some x -> x in
            UidM.add uid (LocSet.add (arg_loc i) prev) c'
          | _ -> c'
        ) arg_pos c
        (* TODO: could also make function output prefer a dead operand location *)
      | _ -> c
    )
    (fun c _ -> c)
    UidM.empty f in 

  List.iter (fun (uid,locs) -> Platform.verb @@ Printf.sprintf "preferred: %s -> %s\n" uid (str_locset locs)) (UidM.bindings preferred_colors);

  let coloring : (Ll.uid * Alloc.loc) list = kempe interference_graph k pre_coloring (UidM.bindings preferred_colors) in (* find k-coloring, try to keep parameters assigned *)

  List.iter (fun (uid,loc) -> Platform.verb @@ Printf.sprintf "allocated: %s <- %s\n" (Alloc.str_loc loc) uid) coloring;

  let lo : (Ll.uid * Alloc.loc) list =
    fold_fdecl
      (fun lo (x, _) -> (x, List.assoc x coloring)::lo)
      (fun lo l -> (l, Alloc.LLbl (Platform.mangle l))::lo)
      (fun lo (x, i) ->
        if insn_assigns i 
        then (x, List.assoc x coloring)::lo
        else (x, Alloc.LVoid)::lo)
      (fun lo _ -> lo)
      [] f in
  { uid_loc = (fun x -> List.assoc x lo)
