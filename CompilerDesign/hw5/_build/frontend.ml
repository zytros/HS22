...
let oat_alloc_struct ct (id:Ast.id) : Ll.ty * operand * stream =
  ...
  let size = Const (List.fold_left (fun a {ftyp=t} -> Int64.add (size_oat_ty t) a) 0L tys) in
  let aty = cmp_ty ct @@ TRef (RStruct id) in
  let arrayTy = Ptr I64 in
  let aIdent = gensym "structID" in
  let arrIdent = gensym "atomStrID" in
  aty, Id aIdent, lift [arrIdent, Call(arrayTy, Gid "oat_malloc", [I64, size]); aIdent, Bitcast(arrayTy, Id arrIdent, aty)]
...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Length e ->
    let arrType, arrOP, sta = cmp_exp tc c e in
    let lenIden = gensym "arraylenIdent" in
    let lenptrIdent = gensym "lenPtrIdent" in
    
    (I64, Id lenIden, sta >@ 
      [I(lenptrIdent, Gep (arrType, arrOP, [Const 0L; Const 0L]))] >@ 
      [I(lenIden, Load (Ptr I64, Id lenptrIdent))])
  ...
  *)
  | Ast.NewArr (elt_ty, e1, id, e2) ->    
    ...
    let arr_ty, arr_op, alloc_code = oat_alloc_array tc elt_ty size_op in
    let arrRefIdent = gensym "arrayRefIdent" in
    let referenceinst = [E(arrRefIdent, Alloca(arr_ty))] >@ [I(gensym "store", Store(arr_ty, arr_op, Id arrRefIdent))] in
    let codeL = no_loc (For (
      [(id, no_loc (CInt 0L))], Some (no_loc (Bop (Lt, no_loc (Id id), no_loc (Length (no_loc (Id arrRefIdent)))))),
      Some (no_loc (Assn (no_loc (Id id), no_loc (Bop (Add, no_loc (Id id), no_loc (CInt 1L)))))),
      [no_loc (Assn (no_loc (Index (no_loc (Id arrRefIdent), no_loc (Id id))), e2))])) in
    let _, codeI = cmp_stmt tc (Ctxt.add c arrRefIdent (Ptr arr_ty, Id arrRefIdent)) Void codeL in
    
    arr_ty, arr_op, size_code >@ alloc_code >@ referenceinst >@ codeI
   ...
   *)
  | Ast.CStruct (id, l) ->
    let structTy, structOP, sta = oat_alloc_struct tc id in
    let tys = TypeCtxt.lookup id tc in
    let iSTAs = List.map (fun {fieldName = n; ftyp = fty} -> 
      let absExp = List.assoc n l in
      let ty, op, sta = cmp_exp tc c absExp in
      let _, lidx = TypeCtxt.lookup_field_name id n tc in
      let ptrIdent = gensym "SgepId" in
      sta >@ [I(ptrIdent, Gep (structTy, structOP, [Const 0L; Const lidx]))] >@ [I(gensym "store", Store (ty, op, Id ptrIdent))] >@ []) tys in
      (structTy, structOP, sta >@ List.concat iSTAs)
...
and cmp_exp_lhs (tc : TypeCtxt.t) (c:Ctxt.t) (e:exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Proj (e, i) ->
    let structTy, structOP, structN = cmp_exp tc c e in
    let structT = 
      match structTy with
      | Ptr (Namedt a) -> a
      | _ -> failwith "failed at: Ast.Proj"
    in
    let ty, lind = TypeCtxt.lookup_field_name structT i tc in
    let identP = gensym "identP" in
    cmp_ty tc ty, Id identP, structN >@ lift [identP, Gep(structTy, structOP, [i64_op_of_int 0; Const lind])]
  ...
  *)
  | Ast.Index (e, i) ->
    ...
    let arr = gensym "arrPIdent" in
    let check = [I(arr, Bitcast (arr_ty, arr_op, Ptr I64))] >@ 
      [I(gensym "check", Ll.Call (Ll.Void, Gid "oat_assert_array_length", 
      [(Ptr I64, Id arr); (I64, ind_op)]))] in



    ans_ty, (Id ptr_id),
    arr_code >@ ind_code >@ check >@ lift
...
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
    let eTy, eOP, eCode = cmp_exp tc c exp in
    let ident = gensym id in
    let cc = Ctxt.add c id (Ptr eTy, Id ident) in
    let nullCode = cmp_block tc c rt null in
    let code = cmp_block tc cc rt notnull in
    let nullIdent = gensym "nullIdent" in
    let nnIdent = gensym "notnullIdent" in
    let cIdent = gensym "cIdent" in
    let castIdent = gensym "castIdent" in
    c, eCode >:: I(castIdent, Icmp (Ne, eTy, eOP, Null)) >:: T(Cbr (Id castIdent, nnIdent, nullIdent)) >:: L nnIdent >@
      [E(ident, Alloca eTy)] >@ [I(gensym "store", Store (eTy, eOP, Id ident))] >@ code >:: T(Br cIdent) >:: L nullIdent >@ 
      nullCode >:: T(Br cIdent) >:: L cIdent
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  ...
  | CStruct (id, cs) ->
    let structTy = Namedt id in
    let ltys = TypeCtxt.lookup id tc in
    let lStas = List.map (fun {fieldName=n; ftyp=fty} -> let absExp = List.assoc n cs in cmp_gexp c tc absExp) ltys in
    let lgdecls, newgdecls = List.split lStas in
    let globalIdent = gensym "CStructIdent" in
    (Ptr structTy, GGid globalIdent), (globalIdent, (structTy, GStruct lgdecls)) :: List.concat newgdecls
